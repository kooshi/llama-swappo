...
func (pm *ProxyManager) ollamaShowHandler() gin.HandlerFunc {
	return func(c *gin.Context) {
		...
		// Resolve template: metadata override takes priority, then architecture-based default
		chatTemplate := ""
		if v, ok := modelCfg.Metadata["template"].(string); ok && v != "" {
			chatTemplate = v
		} else {
			chatTemplate = GetArchitectureTemplate(arch)
			if chatTemplate == "" {
				// Fall back to ChatML for unknown architectures so clients always get a usable template
				chatTemplate = GetArchitectureTemplate("qwen2")
			}
		}

		// Resolve system prompt from metadata
		systemPrompt := ""
		if v, ok := modelCfg.Metadata["system"].(string); ok {
			systemPrompt = v
		}
		...
		if parsedArgs.CmdModelPath != "" {
			modelInfo["general.basename"] = strings.TrimSuffix(parsedArgs.CmdModelPath, ".gguf")
		} else {
			modelInfo["general.basename"] = id
		}
		if ctxLength > 0 {
		...
		if paramCount := parseParameterCount(paramSize); paramCount > 0 {
			modelInfo["general.parameter_count"] = paramCount
		}

		// Build a basic modelfile so clients can parse it
		var sb strings.Builder
		fmt.Fprintf(&sb, "# Modelfile generated by llama-swap\nFROM %s\n", id)
		if chatTemplate != "" {
...
// parseParameterCount converts a human-readable parameter size string (e.g. "7B", "13B", "8x7B")
// to an approximate integer count. Returns 0 if the string cannot be parsed.
func parseParameterCount(paramSize string) int64 {
	if paramSize == "" || paramSize == "unknown" {
		return 0
	}
	s := strings.ToUpper(strings.TrimSpace(paramSize))

	// Handle MoE format like "8x7B"
	if idx := strings.Index(s, "X"); idx != -1 {
		parts := strings.SplitN(s, "X", 2)
		if len(parts) == 2 {
			experts, err1 := parseScaledNumber(parts[0])
			params, err2 := parseScaledNumber(parts[1])
...
func parseScaledNumber(s string) (int64, error) {
...
type OllamaShowRequest struct {
	...
	Name    string `json:"name,omitempty"`  // Deprecated: use Model instead
...
type OllamaShowResponse struct {
	...
	ModelInfo     map[string]any     `json:"model_info"` // not omitempty: several integrations crash on null model_info
